...
const EditorComponent: React.FC<EditorProps> = ({
  ...
  return (
    <div className="flex min-h-screen flex-col items-center pb-0 md:pb-20">
      <Slate editor={editor} initialValue={EMPTY_VALUE}>
        <Toolbar />
        <Cursors>
          <div className="relative min-h-screen w-full max-w-4xl bg-white p-8">
            <Editable
              readOnly={!canEdit}
              renderElement={renderElement}
              renderLeaf={renderLeaf}
              placeholder="Start writing..."
              className={`ring-0 outline-none focus:ring-0 focus:outline-none focus-visible:outline-none focus-visible:ring-0${!canEdit ? "cursor-default select-none" : ""}`}
              spellCheck={false}
              autoFocus={canEdit}
